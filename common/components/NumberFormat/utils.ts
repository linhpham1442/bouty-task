 ...
 * Not used .fixedTo because that will break with big numbers
 */
export function limitToScaleDynamic(numStr: string, scale: number, fixedDecimalScale: boolean) {
  let str = "";
  const filler = fixedDecimalScale ? "0" : "";
  for (let j = 0; j < numStr.length; j++) {
    if (numStr[j] !== "0") {
      for (let i = 0; i <= scale - 1; i++) {
        str += numStr[i + j] || filler;
      }
      break;
    }
    str += "0";
...
export function roundToPrecision(
  ...
  const floatValueStr = toNumericString(floatValue);
  ...
  let decimalPart = roundedDecimalParts[1];
  if (dynamicZero) {
    decimalPart = limitToScaleDynamic(
      roundedDecimalParts[1] || "",
      Math.min(scale, afterDecimal.length),
      fixedDecimalScale
    );
  } else {
    decimalPart = limitToScale(
      roundedDecimalParts[1] || "",
      Math.min(scale, afterDecimal.length),
      fixedDecimalScale
    );
...
export async function getAvgBlockTime() {
  ...
  return (
    ((await web3.eth.getBlock(block)).timestamp -
      (await web3.eth.getBlock(block - 500)).timestamp) /
    500
  );
}
